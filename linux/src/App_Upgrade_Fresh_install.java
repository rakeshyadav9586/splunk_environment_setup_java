...
public class App_Upgrade_Fresh_install {
 static String Linux;
 public static void App_Fresh_upgrade_intall_in_Linux(String ip, String pwd) {
  ...
  System.out.print("Copy latest build " + build_name + " on " + ip);
  try {
  ...
  String command = splunk_stop + "&&" + untar_spl + "&&" + remove_spl + "&&" + splunk_start;
  //System.out.println(command);
  try {
   FileWriter fileWriter = new FileWriter("App_Upgrade_Fresh_install.txt");
   ...
   channel.disconnect();
   session.disconnect();
   System.out.println("Splunk Nurse app installed on Linux ip : " + ip);
   System.out.println();
   printWriter.close();
  } catch (Exception e) {
   e.printStackTrace();
 ...
 public static void main(String[] args) throws AWTException {
  ...
  App_Fresh_upgrade_intall_in_Linux("10.0.0.0", "admin");
 }
}